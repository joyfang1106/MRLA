...
__all__ = [
    'deit_mrlab_tiny_patch16_224', 
    'deit_mrlab_small_patch16_224', 
    'deit_mrlab_base_patch16_224',
]
...
class mrlab_layer(nn.Module):
    """
    multi-head recurrent layer attention module: base version
    ...
    def __init__(self, input_dim, heads=None, dim_perhead=None, k_size=None, init_cell=False):
        super(mrlab_layer, self).__init__()
...
class mrlab_module(nn.Module):
    # dim_perhead = 16
    
    def __init__(self, input_dim, dim_perhead, 
                 init_cell=False, channel_wise=False, 
                 norm_layer=partial(nn.LayerNorm, eps=1e-6)):
        super(mrlab_module, self).__init__()
        ...
        # if not self.init_cell:
        #     self.normk = norm_layer(input_dim)
        #     self.normv = norm_layer(input_dim)
        
        self.mrla = mrlab_layer(input_dim=input_dim, dim_perhead=self.dim_perhead, init_cell=init_cell)
    ...
    def forward(self, xt, prev_k, prev_v):
        ...
        if self.init_cell: # 1st layer in each stage
           ...
           prev_v = None 
        # else:
        #     prev_k = self.normk(prev_k)
        #     prev_v = self.normv(prev_v)
        
        b, n, c = xt.size()
        cls_token, tokens = torch.split(xt, [1, n - 1], dim=1)
        # h, w = int(math.sqrt(n - 1)), int(math.sqrt(n - 1))
        xt = tokens.reshape(b, int(sqrt(n - 1)), int(sqrt(n - 1)), c).permute(0, 3, 1, 2)
        
        xt, kt, vt = self.mrla(xt, prev_k, prev_v)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, 
                 dim_mrla, init_cell=False, 
                 layer_index=0, mrlab_size=4, 
        ...
        if layer_index % mrlab_size == 0:
            init_cell = True
        else:
            init_cell = False
        self.mrla = mrlab_module(input_dim=dim, dim_perhead=dim_mrla, init_cell=init_cell)
    ...
    def forward(self, x, prev_k, prev_v):
        ...
        # layer attention
        attn_t, k, v = self.mrla(x, prev_k, prev_v)
        x = x + attn_t
        
        return x, k, v


class ViT_mrlab(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, 
        ...
        self.blocks.append(
            Block(
                ...
                dim_mrla=dim_mrla, init_cell=True, 
                layer_index=0, mrlab_size=4, 
                mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        ...
        for i in range(1, depth):
            self.blocks.append(
                Block(
                    dim=embed_dim, num_heads=num_heads, 
                    dim_mrla=dim_mrla, init_cell=False, 
                    layer_index=i, mrlab_size=4, 
                    mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
...
@register_model
def deit_mrlab_tiny_patch16_224(pretrained=False, **kwargs):
    model = ViT_mrlab(
...
@register_model
def deit_mrlab_small_patch16_224(pretrained=False, **kwargs):
    model = ViT_mrlab(
...
@register_model
def deit_mrlab_base_patch16_224(pretrained=False, **kwargs):
    model = ViT_mrlab(
...
if __name__ == '__main__':
    ...
    compute_params(deit_mrlab_tiny_patch16_224())