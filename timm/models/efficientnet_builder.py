...
from .efficientnet_blocks import InvertedResidual_CRLA, InvertedResidual_MRLA
...
__all__ = ["EfficientNetBuilder", "decode_arch_def", "efficientnet_init_weights",
           ...
           "EfficientNetBuilder_CRLA", "EfficientNetBuilder_MRLA"]
...
class EfficientNetBuilder_CRLA:
    """ Build Trunk Blocks
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            if ba.get('num_experts', 0) > 0:
                ...
                block = InvertedResidual_CRLA(**ba)
        elif bt == 'ds' or bt == 'dsa':
            _log_info_if('  DepthwiseSeparable {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = DepthwiseSeparableConv(**ba)
        elif bt == 'er':
...
class EfficientNetBuilder_MRLA:
    """ Build Trunk Blocks
    ...
    def _make_block(self, ba, block_idx, block_count):
        ...
        if bt == 'ir':
            ...
            if ba.get('num_experts', 0) > 0:
                ...
                block = InvertedResidual_MRLA(**ba)
        elif bt == 'ds' or bt == 'dsa':
            _log_info_if('  DepthwiseSeparable {}, Args: {}'.format(block_idx, str(ba)), self.verbose)
            block = DepthwiseSeparableConv(**ba)
        elif bt == 'er':
